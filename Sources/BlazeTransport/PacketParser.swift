...
public struct PacketParser {
    ...
    public static func decode(_ data: Data) throws -> BlazePacket {
        ...
        // Manual byte reads already produce native-endian values; no further swap needed.
        let connectionID = UInt32(data[offset]) << 24 | UInt32(data[offset + 1]) << 16 | UInt32(data[offset + 2]) << 8 | UInt32(data[offset + 3])
        offset += 4

        let packetNumber = UInt32(data[offset]) << 24 | UInt32(data[offset + 1]) << 16 | UInt32(data[offset + 2]) << 8 | UInt32(data[offset + 3])
        offset += 4

        let streamID = UInt32(data[offset]) << 24 | UInt32(data[offset + 1]) << 16 | UInt32(data[offset + 2]) << 8 | UInt32(data[offset + 3])
        offset += 4

        let payloadLength = UInt16(data[offset]) << 8 | UInt16(data[offset + 1])
        offset += 2
